...
const MOCK_DASHBOARD = {
  ...
  recent_trades: [
    { trade_id: 'TRD-0491', timestamp: '2026-04-11T10:15:00', symbol: 'RELIANCE', trade_type: 'BUY', quantity: 10, pnl: 1200, behavioral_pattern: 'normal', outcome: 'WIN', emotion_tag: { label: 'Normal', color: '#10B981', severity: 0 } },
    { trade_id: 'TRD-0492', timestamp: '2026-04-11T10:32:00', symbol: 'TCS', trade_type: 'SELL', quantity: 5, pnl: -3400, behavioral_pattern: 'normal', outcome: 'LOSS', emotion_tag: { label: 'Normal', color: '#10B981', severity: 0 } },
    { trade_id: 'TRD-0493', timestamp: '2026-04-11T10:38:00', symbol: 'BANKNIFTY', trade_type: 'BUY', quantity: 25, pnl: -6800, behavioral_pattern: 'revenge_trade', outcome: 'LOSS', emotion_tag: { label: 'Revenge Trade', color: '#EF4444', severity: 5 } },
    { trade_id: 'TRD-0494', timestamp: '2026-04-11T11:05:00', symbol: 'INFY', trade_type: 'BUY', quantity: 15, pnl: 2100, behavioral_pattern: 'normal', outcome: 'WIN', emotion_tag: { label: 'Normal', color: '#10B981', severity: 0 } },
    { trade_id: 'TRD-0495', timestamp: '2026-04-11T11:22:00', symbol: 'SBIN', trade_type: 'BUY', quantity: 20, pnl: -1800, behavioral_pattern: 'fomo_trade', outcome: 'LOSS', emotion_tag: { label: 'FOMO', color: '#F59E0B', severity: 3 } },
  ],
};

// ── Demo mode scenario steps ──
const DEMO_STEPS = [
  { delay: 0, riskScore: 32, label: 'SAFE', color: '#10B981', event: null },
  {
    delay: 3000, riskScore: 58, label: 'CAUTION', color: '#F59E0B',
    event: {
      type: 'trade',
      trade: { symbol: 'RELIANCE', trade_type: 'SELL', pnl: -4200, behavioral_pattern: 'normal', outcome: 'LOSS', quantity: 15, emotion_tag: { label: 'Normal', color: '#10B981' } },
      toast: '📉 Trade executed: RELIANCE SELL — Loss ₹4,200'
    }
  },
  {
    delay: 6000, riskScore: 87, label: 'DANGER', color: '#DC2626',
    event: {
      type: 'cooldown',
...
export default function App() {
  ...
  const getRiskBg = (score) => {
    if (score < 30) return 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30';
  ...
  return (
    <AppContext.Provider value={contextValue}>
      <div className="flex h-screen overflow-hidden bg-[#0F0F14]">
        {/* ═══ Sidebar ═══ */}
        <aside className="w-[240px] bg-[#0F0F14] border-r border-[#2A2A3A] flex flex-col py-5 px-4 shrink-0">
          {/* Logo */}
          <div className="flex items-center gap-3 px-2 mb-8">
            ...
            <div>
              <h1 className="text-sm font-bold tracking-wide text-white">SENTINEL</h1>
              <p className="text-[10px] text-slate-500 tracking-wider">BEHAVIORAL SHIELD</p>
            </div>
          </div>
          ...
          {/* Demo mode button */}
          <button onClick={startDemo} className="flex items-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-sm font-semibold hover:from-indigo-500 hover:to-purple-500 transition-all shadow-lg shadow-indigo-500/20 mt-4">
            <Play size={16} />
            Live Demo Mode
          </button>

          {/* User profile */}
          <div className="flex items-center gap-3 px-2 pt-5 mt-4 border-t border-[#2A2A3A]">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center">
              <User size={14} className="text-white" />
            </div>
            <div>
              <p className="text-xs font-semibold text-slate-200">Rahul Sharma</p>
              <p className="text-[10px] text-slate-500">Retail Trader</p>
            </div>
          </div>
        </aside>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* Top Bar */}
          <header className="h-14 bg-[#0F0F14]/80 backdrop-blur-md border-b border-[#2A2A3A] flex items-center justify-between px-6 shrink-0 z-10">
            <div className="flex items-center gap-3">
              <Activity size={16} className="text-indigo-400" />
              <span className="text-sm font-medium text-slate-300">
                {demoMode && <span className="text-indigo-400 font-semibold mr-2">● DEMO MODE</span>}
                {activePage === 'dashboard' && 'Trading Dashboard'}
                {activePage === 'stress-test' && 'Portfolio Stress Test'}
                {activePage === 'tip-checker' && 'Misinformation Shield'}
                {activePage === 'ai-coach' && 'AI Finance Coach'}
              </span>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-1 text-xs">
                <TrendingUp size={12} className={dashboardData?.pnl_today >= 0 ? 'text-emerald-400' : 'text-red-400'} />
                <span className={dashboardData?.pnl_today >= 0 ? 'text-emerald-400' : 'text-red-400'}>
          ...
          <main className="flex-1 overflow-y-auto p-6 scrollbar-thin">
            {/* Toast notification */}
            {toastMessage && (
              <div className="fixed top-4 right-4 z-50 bg-[#1A1A24] border border-[#2A2A3A] rounded-xl px-5 py-3 shadow-2xl fade-in-up text-sm text-slate-200 flex items-center gap-3 max-w-sm">
                {toastMessage}
              </div>
            )}
            ...
            {/* Savings message */}
            {savingsMessage && (
              <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-6 py-4 shadow-2xl fade-in-up text-sm text-emerald-300 font-semibold max-w-md text-center">
                🎉 {savingsMessage}
              </div>
            )}
