...
export default function CoolDownCard({ data, onCancel, onProceed }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center cooldown-overlay">
      ...
      {/* Main card */}
      <div className="relative w-full max-w-lg mx-4 bg-[#13131A] border-2 border-red-500/40 rounded-3xl p-8 cooldown-card overflow-y-auto max-h-[90vh]">

        {/* ── Header ── */}
        <div className="flex flex-col items-center text-center mb-6">
          <div className="w-16 h-16 rounded-2xl bg-red-500/20 flex items-center justify-center mb-4 danger-pulse">
            <ShieldAlert size={32} className="text-red-400" />
          </div>
          <h2 className="text-2xl font-black text-white tracking-tight">
            HOLD ON, RAHUL
          </h2>
          <p className="text-sm text-slate-400 mt-1">SENTINEL has detected a dangerous pattern</p>
        </div>

        {/* ── Risk Score ── */}
        <div className="flex items-center justify-center gap-3 mb-5">
          <span className="text-sm text-slate-400 font-medium">Risk Score:</span>
          <span className="text-3xl font-black text-red-400">{pattern.risk_score || 87}</span>
          <span className="text-lg text-slate-500 font-semibold">/100</span>
        </div>

        {/* ── Pattern Detection ── */}
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 text-center mb-5">
          <p className="text-xs text-red-400 font-bold uppercase tracking-widest mb-1">Detected Pattern</p>
          <p className="text-xl font-black text-red-300">
            {patternDisplay[pattern.pattern] || 'EMOTIONAL TRADE'} DETECTED
          </p>
          <p className="text-xs text-slate-400 mt-1">
            Attempting: {trade.symbol} {trade.trade_type} · ₹{(trade.position_size_inr || 0).toLocaleString('en-IN')}
          </p>
        </div>
        ...
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 mb-5">
          <p className="text-xs text-amber-400 font-bold uppercase tracking-wider mb-3">
            Last {pattern.historical_count || 8} times you traded in this pattern:
          </p>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="text-red-400 font-bold">✗</span>
              <span className="text-sm text-slate-300">
                Win rate: <span className="text-red-400 font-bold">{((pattern.historical_win_rate || 0.12) * 100).toFixed(0)}%</span>
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-red-400 font-bold">✗</span>
              <span className="text-sm text-slate-300">
                Average loss: <span className="text-red-400 font-bold">{formatINR(Math.abs(pattern.historical_avg_loss || 6400))}</span>
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-red-400 font-bold">✗</span>
              <span className="text-sm text-slate-300">
                Total lost in this pattern: <span className="text-red-400 font-bold">{formatINR(Math.abs(pattern.historical_total_loss || 51200))}</span>
              </span>
            </div>
          </div>
        </div>
            ...
            <>
              ...
              <div className="relative w-28 h-28 mb-4">
                <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                  <circle cx="50" cy="50" r="45" fill="none" stroke="#1A1A24" strokeWidth="5" />
              ...
              <div className="flex flex-col items-center gap-3">
                <div className="relative w-16 h-16">
                  <div className={`absolute inset-0 rounded-full border-2 border-indigo-400/50 breathe-circle`}
                    style={{
                ...
                <div className="text-center">
                  <p className="text-sm font-semibold capitalize" style={{
                    color: breathPhase === 'inhale' ? '#818CF8' : breathPhase === 'hold' ? '#F59E0B' : '#10B981'
                  }}>
                    {breathPhase === 'inhale' ? '🫁 Breathe In' : breathPhase === 'hold' ? '⏸ Hold' : '💨 Breathe Out'}
                  </p>
                  <p className="text-xs text-slate-500">{breathCount}s</p>
                </div>
              </div>
            </>
          ) : (
        ...
        {/* AI Message */}
        {pattern.message && (
          <div className="mt-2 p-3 rounded-xl bg-[#1A1A24] border border-[#2A2A3A]">
